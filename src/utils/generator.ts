...
export function generatePassword(length: number, included: string[]) {
  ...
  included.forEach((type) => {
    password += getRandomCharFromString(Allowed[type as AllowedType])
  })

  // Fill the rest with random characters
  const allowed = included.map((type) => Allowed[type as AllowedType]).join("")
...
export function getPasswordEntropy(length: number, included: string[]) {
  ...
  const poolSize = included
    .map((type) => Allowed[type as AllowedType])
    .join("").length
